...
public sealed class GoogleDriveOperations : IDriveOperations
{
    ...
    public async Task<IReadOnlyList<DriveItem>> ListFolderAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        do
        {
            ...
            var page = await response.Content.ReadFromJsonAsync(AppJsonContext.Default.GoogleDriveFilesPage, cancellationToken)
                ?? throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, "Google Drive returned an empty listing page.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpEmptyListingPage, "Google Drive") };
    ...
    private async Task<string> ReserveKeepBothNameAsync(string parentId, string name, CancellationToken cancellationToken)
    {
        ...
        throw new DriveException($"POST files (create {name})", 0, string.Empty, string.Empty,
            $"No free name was found for '{name}' on Google Drive after 1000 attempts.", DriveErrorKind.AlreadyExists) { Detail = LocalizedText.Of(StringKeys.Error.OpNoFreeName, "Google Drive", name) };
    }
    ...
    private async Task<GoogleDriveFile> UploadMultipartAsync(string localPath, FileInfo fileInfo, string parentId, string name, string? existingId, CancellationToken cancellationToken)
    {
        ...
        return await response.Content.ReadFromJsonAsync(AppJsonContext.Default.GoogleDriveFile, cancellationToken)
            ?? throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, $"Google Drive did not return the uploaded file for {name}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoUploadedFile, "Google Drive", name) };
    }
    ...
    private async Task<GoogleDriveFile> UploadResumableAsync(string localPath, FileInfo fileInfo, string parentId, string name, string? existingId, CancellationToken cancellationToken)
    {
        ...
        var sessionUri = initiateResponse.Headers.Location?.ToString()
            ?? throw new DriveException(initiateUrl, (int)initiateResponse.StatusCode, string.Empty, string.Empty, $"Google Drive did not return a resumable upload session for {name}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoResumableSession, "Google Drive", name) };
        ...
        try
        {
            while (offset < totalLength)
            {
                ...
                if (!lastResponse.IsSuccessStatusCode)
                {
                    ...
                    throw new DriveException(sessionUri, (int)lastResponse.StatusCode, string.Empty, body,
                        $"The upload of {name} to Google Drive failed at byte {offset}.", GoogleDriveErrorClassifier.Classify(lastResponse.StatusCode, body)) { Detail = LocalizedText.Of(StringKeys.Error.OpUploadFailedAtByte, "Google Drive", name, offset) };
                }
            ...
            return lastResponse is not null
                ? await lastResponse.Content.ReadFromJsonAsync(AppJsonContext.Default.GoogleDriveFile, cancellationToken)
                    ?? throw new DriveException(sessionUri, 0, string.Empty, string.Empty, $"Google Drive did not return the uploaded file for {name}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoUploadedFile, "Google Drive", name) }
                : throw new DriveException(sessionUri, 0, string.Empty, string.Empty, $"'{name}' was empty; no chunk was ever uploaded.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpEmptyUpload, name) };
        }
        finally
        {
            lastResponse?.Dispose();
    ...
    public async Task CreateFolderAsync(string parentPath, string name, CancellationToken cancellationToken = default)
    {
        ...
        var created = await response.Content.ReadFromJsonAsync(AppJsonContext.Default.GoogleDriveFile, cancellationToken)
            ?? throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, $"Google Drive did not return the created folder {name}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoCreatedFolder, "Google Drive", name) };
    ...
    public async Task CopyItemAsync(string sourcePath, string targetParentPath, string? newName = null, CancellationToken cancellationToken = default)
    {
        ...
        var copied = await response.Content.ReadFromJsonAsync(AppJsonContext.Default.GoogleDriveFile, cancellationToken)
            ?? throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, $"Google Drive did not return the copy of {sourcePath}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoCopy, "Google Drive", sourcePath) };
    ...
    public async Task<string> CreateShareLinkAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        return file?.WebViewLink is { Length: > 0 } webViewLink
            ? webViewLink
            : throw new DriveException(fileUrl, (int)fileResponse.StatusCode, string.Empty, string.Empty, $"Google Drive did not return a sharing link for {path}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoShareLink, "Google Drive", path) };
    }
    ...
    private async Task<string> GetCurrentParentIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        return file?.Parents is { Count: > 0 } parents
            ? parents[0]
            : throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, $"Google Drive did not return a parent for item {id}.", DriveErrorKind.Unknown) { Detail = LocalizedText.Of(StringKeys.Error.OpNoParent, "Google Drive", id) };
    }
    ...
    private async Task<string> ResolveIdAsync(string path, CancellationToken cancellationToken)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            var match = page?.Files.FirstOrDefault()
                ?? throw new DriveException(url, (int)response.StatusCode, string.Empty, string.Empty, $"'{segment}' was not found on Google Drive inside {currentPath}.", DriveErrorKind.NotFound) { Detail = LocalizedText.Of(StringKeys.Error.OpSegmentNotFound, "Google Drive", segment, currentPath) };
