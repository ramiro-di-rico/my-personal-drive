...
public interface IProviderCatalog
{
    ...
    /// reaching here is a bug in whatever chose it, not a case to degrade gracefully from. Callers
    /// reading an id out of persisted settings should go through <see cref="ResolveOrDefault"/>
    /// first, precisely so they never reach this exception.
    /// </summary>
    ICloudDriveProvider Create(ProviderId id, AppSettingsService settings);

    /// <summary>
    /// <paramref name="requested"/> if it's actually buildable (present in <see cref="Available"/>),
    /// otherwise the catalog's own default. Completes the "degrade rather than throw at startup"
    /// promise <c>AppSettings.ActiveProviderOrDefault</c> makes but can't keep alone: that method
    /// only catches a string this build has never heard of, not a name that parses to a real
    /// <see cref="ProviderId"/> with no catalog entry (e.g. <c>ProviderId.OneDrive</c> before P6
    /// implements it) — only the catalog knows what it can actually construct.
    /// </summary>
    ProviderId ResolveOrDefault(ProviderId requested);
}
