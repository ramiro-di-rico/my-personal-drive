...
public class CliErrorClassifierTests
{
    ...
    {
        Assert.Equal(DriveErrorKind.NotAuthenticated, CliErrorClassifier.Classify(1, "", stderr));
    }
    ...
    {
        Assert.Equal(DriveErrorKind.NotFound, CliErrorClassifier.Classify(1, "", stderr));
    }

    [Fact]
    public void DetectsAlreadyExists()
    {
        Assert.Equal(DriveErrorKind.AlreadyExists, CliErrorClassifier.Classify(1, "", "A folder with that name already exists"));
    }

    [Fact]
    public void UnrecognizedMessage_IsUnknown()
    {
        Assert.Equal(DriveErrorKind.Unknown, CliErrorClassifier.Classify(1, "", "Something went sideways"));
    }

    [Fact]
    public void FallsBackToStdoutWhenStderrIsEmpty()
    {
        Assert.Equal(DriveErrorKind.NotFound, CliErrorClassifier.Classify(1, "the requested path was not found", ""));
    }
    ...
    public void DetectsTheCliLosingARaceOnItsOwnSqliteCache()
    {
        ...
        Assert.Equal(DriveErrorKind.Busy, CliErrorClassifier.Classify(1, "code: \"SQLITE_BUSY\"", ""));
        Assert.Equal(DriveErrorKind.Busy, CliErrorClassifier.Classify(1, "", "database is locked"));
    }
    ...
    public void ClassifiesUsingStdoutWhenStderrIsJustTheCrashBanner()
    {
        ...
        const string banner = "===============================================\n";
        Assert.Equal(DriveErrorKind.Busy, CliErrorClassifier.Classify(1, "{ code: 'SQLITE_BUSY', errno: 5 }", banner));
    }

    [Fact]
    public void ClassificationIsCaseInsensitive()
    {
        Assert.Equal(DriveErrorKind.NotAuthenticated, CliErrorClassifier.Classify(1, "", "LOGIN FIRST"));
    }
}
